...
```{r}
homeworkData <- data.frame(姓名=c("小明","小花","小新"),
                             作業1分數=c(10,9,5),
                             作業2分數=c(10,10,9),stringsAsFactors=FALSE)
homeworkData
```
...
str_smallClosest <- function(chrNumber,criticalNumber){
  as.numeric(chrNumber) -> chrNumber
  logiTrue <- chrNumber <= criticalNumber
  max(chrNumber[logiTrue]) -> maxNum
  return(as.character(maxNum))
}
str_smallClosest(c(2,3,5,7,6),6)
str_smallClosest
```
...
```{r}
EconCourseSelectionInfo <- list("104"=list(url="http://www.econ.ntpu.edu.tw/econ/files/Way/2016072195041.doc"),
...
search_courseRules <- function(entranceYear,EconCourseSelectionInfo){
  ...
  else if(input>=105 & input<106){
    output <- EconCourseSelectionInfo[["105"]]$url}
  else if(input>=106 & input<107){
    output <- EconCourseSelectionInfo[["105"]]$url}
  else if(input>=107){
...
search_courseRules_flexible(101,courseSelectionInfo)
```
...
```{r}
EconCourseSelectionInfo -> EconCourseSelectionInfo2
...
invoiceList$UTC時間[nyLogical] <-
  with_tz(ymd_hms((invoiceList$訂單時區之下單時間)[nyLogical],tz="America/New_York"),tzone="UTC")
invoiceList$UTC時間[tpLogical] <-
  with_tz(ymd_hms(invoiceList$訂單時區之下單時間[tpLogical],tz="Asia/Taipei"),tzone="UTC")
invoiceList
```
...
```{r}
(str_sub(sampledTranscriptData$學號,5,6)!=73 & str_detect(sampledTranscriptData$系級,"(經濟學系)")) -> logiCheck
